...
st.title("RAG Chat")
st.write("A minimal chat interface that queries the repository's RAG workflow")
...
with st.sidebar:
    ...
    collection = st.text_input("collection", value=st.session_state.get("collection", "bfp-a3447q_hybrid"))
...
def send_query(user_text: str):
    ...
    collection = st.session_state.get("collection", "bfp-a3447q")
    try:
